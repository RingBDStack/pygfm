...
def get_chem_id2name():
    with open(_chem_disk() / "id2element.csv", "r") as file1:
        Lines = file1.readlines()
    chem_dict_local = {}
    for line in Lines:
        line_split = line.strip().split(",")
        chem_dict_local[line_split[0]] = line_split[2]
    return chem_dict_local
...
def get_raw_graphs(data_path):
    arr = torch_load_compat(
        str(_chem_disk() / "chembl_pretrain.pth"),
    )
    graphs = []
