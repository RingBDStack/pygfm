...
from pygfm.baseline_models.oneforall.data.chemmol.gen_raw_graph import smiles2graph
from pygfm.baseline_models.oneforall.data.ofa_data import OFAPygDataset
from pygfm.baseline_models.oneforall.paths import get_data_root, get_molecule_dataset_cache_dir
from pygfm.baseline_models.oneforall.utils import get_label_texts

NAME_TO_SPLIT = {"chemblpre": "chembl_pretraining", "chempcba": "pcba", "chemhiv": "hiv"}


def _chem_disk():
    return get_data_root() / "chemmol"


def load_prompt_json(name):
    if name == "chemblpre":
        with open(_chem_disk() / "prompt_pretrain.json", "rb") as f:
            prompt_text = json.load(f)
        return prompt_text["chembl"]
    else:
        with open(_chem_disk() / "mol_label_desc.json", "rb") as f:
            prompt_text = json.load(f)
...
def get_local_text(name):
    print("gen text")
    cache_dir = str(get_molecule_dataset_cache_dir())
