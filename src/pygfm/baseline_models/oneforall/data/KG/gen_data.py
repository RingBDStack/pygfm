...
def gen_entities(name):
    ...
    if name == "WN18RR":
        ...
        entity_lst = []
        text_lst = []
        with open(kg / name / "entity2text.txt", "r") as f:
            lines = f.readlines()
            for line in lines:
        ...
        entity_lst = []
        text_lst = []
        with open(kg / name / "entity2wikidata.json", "r") as f:
            data = json.load(f)
...
class KGOFADataset(OFAPygDataset):
    def gen_data(self):
        ...
        name_dict = {n: kg / self.name / (n + ".txt") for n in names}
