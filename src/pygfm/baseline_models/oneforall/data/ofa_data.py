...
from pygfm.baseline_models.oneforall.paths import get_cache_root
from pygfm.baseline_models.oneforall.utils import SentenceEncoder
...
class OFAPygDataset(InMemoryDataset, ABC):
    ...
    def __init__(self, name: str, load_texts: bool, encoder: Optional[SentenceEncoder] = None,
                 root: Optional[str] = None, transform: Optional[Callable] = None,
...
class OFAPygSTDataset(OFAPygDataset):
    def __init__(self, name, encoder, root=None, load_text=False, load_feat=True, transform=None,
        ...
        if load_text:
            self.texts = torch_load_compat(self.processed_paths[0])
    ...
    def get(self, idx):
        data = torch_load_compat(self.processed_paths[idx + 3])
        return data
