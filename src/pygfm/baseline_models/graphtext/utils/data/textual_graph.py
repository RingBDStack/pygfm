...
from utils.data.pyg_graph import PyGGraph, to_bidirected_and_self_loop
...
def _prepare_ogb_cache(
    ...
    # ! Process Full Graph
    pyg_data, labels, split_idx = load_ogb_graph_structure_only(
    ...
    if sample_per_class > 0 or subset_class != n_labels:
        ...
        g, split_ids = subset_graph(
            pyg_data, sample_per_class, split_ids, labels, label_subset
        )
        pyg_data = g._data if isinstance(g, PyGGraph) else g
    ...
    if sample_per_class > 0 or subset_class != n_labels:
        g_info.IDs = pyg_data["_ID"].cpu().numpy() if isinstance(pyg_data["_ID"], th.Tensor) else np.asarray(pyg_data["_ID"])
...
def preprocess_ogb(ogb_name, process_mode, raw_data_path, raw_text_url, max_seq_len, info_file, processed_text_file,
    ...
    pyg_data = load_ogb_graph_structure_only(ogb_name, raw_data_path, graph_save_path)[0]
    for ndata_field, data_file in additional_ndata.items():
        pyg_data[ndata_field] = th.load(data_file)
    pyg_data = to_bidirected_and_self_loop(pyg_data)
    ...
    # Keep a DGL-like label field for downstream code that expects g.ndata['label'].
    g.ndata["label"] = g.ndata["y"].long()
...
def subset_graph(
    ...
    if sample_per_class > 0 or label_subset != len(np.unique(labels)):
        ...
        pyg = g._data if isinstance(g, PyGGraph) else g
        node_subset_t = th.as_tensor(node_subset, dtype=th.long)
        ...
        # Map old node ids -> new [0..n_sub-1]
        old_to_new = -th.ones(pyg.num_nodes, dtype=th.long)
        old_to_new[node_subset_t] = th.arange(node_subset_t.numel(), dtype=th.long)
        split_ids = {
            k: old_to_new[th.as_tensor(v, dtype=th.long)].cpu().numpy()
            for k, v in split_ids.items()
        }
        data_sub = Data(
            x=getattr(pyg, "x", None)[node_subset_t] if getattr(pyg, "x", None) is not None else None,
            y=getattr(pyg, "y", None)[node_subset_t] if getattr(pyg, "y", None) is not None else None,
            edge_index=edge_index_sub,
            num_nodes=int(node_subset_t.numel()),
        )
        # Keep original IDs so downstream can recover labels/text.
        data_sub["_ID"] = node_subset_t
        g = PyGGraph(data_sub)

    return g, split_ids


def preprocess_torch_pt(data_cfg: DictConfig):
    """Load a ``.pt`` file with tensors (or a single ``torch_geometric.data.Data``).

    Supported on disk:

    - ``torch.save((dict, ...), path)`` where dict has ``x``, ``edge_index``, ``y``, and optional
      ``train_mask`` / ``val_mask`` / ``test_mask`` (Planetoid-style bool tensors).
    - A plain ``dict`` with the same keys.
    - A ``Data`` object.

    Label names and node text follow ``data_cfg`` (same as ``type: pyg``), e.g. ``label_name`` +
    ``text.mode: label_name``.
    """
    from torch_geometric.data import Data

    path = data_cfg.get("dataset_path")
    if not path:
        raise ValueError("data.type=torch_pt requires data.dataset_path (e.g. ${path.data_pt}Cora.pt)")
    raw = th.load(path, map_location="cpu")
    if isinstance(raw, (list, tuple)):
        raw = next((x for x in raw if x is not None), None)
    if raw is None:
        raise ValueError(f"torch_pt: nothing to load from {path}")
    if isinstance(raw, dict):
        ...
        pyg_data = Data(
            ...
            y=y,
            train_mask=raw.get("train_mask"),
            val_mask=raw.get("val_mask"),
            test_mask=raw.get("test_mask"),
        )
    elif hasattr(raw, "edge_index") and hasattr(raw, "x"):
        pyg_data = raw
        if pyg_data.y.dim() > 1:
            pyg_data.y = pyg_data.y.squeeze()
    else:
        raise ValueError(f"torch_pt: unsupported object type {type(raw)} in {path}")
    for s in ["train", "val", "test"]:
        mask = getattr(pyg_data, f"{s}_mask", None)
        if mask is not None and mask.dim() > 1:
            setattr(pyg_data, f"{s}_mask", mask[:, 0])
    for s in ["train", "val", "test"]:
        if getattr(pyg_data, f"{s}_mask", None) is None:
            raise ValueError(
                f"torch_pt: missing {s}_mask in {path}; provide Planetoid-style boolean masks."
            )
    labels = pyg_data.y.numpy() if pyg_data.y.dim() == 1 else pyg_data.y.squeeze().numpy()
    split_ids = EasyDict({
        s: np.random.permutation(np.where(getattr(pyg_data, f"{s}_mask").numpy())[0])
        for s in ["train", "val", "test"]
    })
    g_info = EasyDict(
        splits=split_ids,
        labels=labels,
        n_nodes=pyg_data.num_nodes,
        IDs=np.arange(pyg_data.num_nodes),
    )
    all_label_info = pd.DataFrame.from_dict({
        "label_id": [int(k) for k in data_cfg.label_name],
        "label_name": list(data_cfg.label_name.values()),
    })
    data = pd.DataFrame.from_dict({"label_id": labels})
    label_info, choice_to_label_id, choice_to_label_name, _ = initialize_label_and_choices(
    ...
    data["pred_choice"] = ""
    return pyg_data, g_info, data, label_info, choice_to_label_id, choice_to_label_name, None


def preprocess_pyg(data_cfg: DictConfig):
    """Load PyG Planetoid/WebKB and return (Data, g_info, text_df, label_info, choice_to_label_id, choice_to_label_name, None)."""
    from torch_geometric.data import Data
    init_args = data_cfg.get("_init_args") or {}
    root = str(data_cfg.get("raw_dir") or init_args.get("raw_dir", "data"))
    name = data_cfg.name
    if name in ("cora", "citeseer", "pubmed"):
        ...
        ds = WebKB(root=root, name=name.capitalize())
        pyg_data = ds[0]
    else:
        raise ValueError(f"preprocess_pyg: unknown dataset name {name}")
    labels = pyg_data.y.numpy() if pyg_data.y.dim() == 1 else pyg_data.y.squeeze().numpy()
    for s in ["train", "val", "test"]:
        mask = getattr(pyg_data, f"{s}_mask", None)
        if mask is not None and mask.dim() > 1:
...
def preprocess_dgl(data_cfg: DictConfig):
    """Legacy DGL path; prefer preprocess_pyg. Redirects to PyG for standard datasets."""
    return preprocess_pyg(data_cfg)
...
def preprocess_explore_llm_on_graph(data_cfg: DictConfig):
    ...
    def _mask(dataset, name):
        for key in (f"{name}_masks", f"{name}_mask"):
            m = getattr(dataset, key, None)
            if m is not None:
                return m[0] if isinstance(m, (list, tuple)) or (hasattr(m, "dim") and m.dim() > 1) else m
        return None
    pyg_data = Data(
        x=dataset.x,
        y=dataset.y.squeeze() if dataset.y.dim() > 1 else dataset.y,
        edge_index=dataset.edge_index,
        train_mask=_mask(dataset, "train"),
        val_mask=_mask(dataset, "val"),
        test_mask=_mask(dataset, "test"),
    )
    ...
    data = pd.DataFrame.from_dict({"label_id": labels})
    label_info, choice_to_label_id, choice_to_label_name, _ = \
    ...
    data["pred_choice"] = ""
    return pyg_data, g_info, data, label_info, choice_to_label_id, choice_to_label_name
...
class TextualGraph:
    ...
    def __init__(self, cfg: DictConfig):  # Process split settings, e.g. -1/2 means first split
        ...
        elif cfg.data.type == "torch_pt":
            pyg_data, self.g_info, text, label_info, c2l, c2n, _ = preprocess_torch_pt(cfg.data)
            self.choice_to_label_id, self.choice_to_label_name = c2l, c2n
            self.g = PyGGraph(to_bidirected_and_self_loop(pyg_data))
        elif cfg.data.type in ("pyg", "dgl"):
            pyg_data, self.g_info, text, label_info, c2l, c2n, _ = preprocess_pyg(cfg.data)
            self.choice_to_label_id, self.choice_to_label_name = c2l, c2n
            self.g = PyGGraph(to_bidirected_and_self_loop(pyg_data))
        else:  # In Exploring the Potential of Large Language Models (LLMs) in Learning on Graphs
            pyg_data, self.g_info, text, label_info, self.choice_to_label_id, self.choice_to_label_name = (
                preprocess_explore_llm_on_graph(cfg.data))
            self.g = PyGGraph(to_bidirected_and_self_loop(pyg_data))
        print({k: len(v) for k, v in self.g_info.splits.items()})

        if "feat" in self.g.ndata:
            self.g.ndata["x"] = self.g.ndata.pop("feat")
        self.g.ndata["y"] = F.one_hot(self.g.ndata["label"].long())
